...
const client = new Discord.Client({ intents: 32767, allowedMentions: { parse: ['users'], repliedUser: true }});
const fs = require("fs");
...
fs.readdir('./commands/penguin/', (err, files) => {
  if (err) return console.log('Could not find any penguin commands!')
  const jsFiles = files.filter(f => f.split('.').pop() === 'js')
  console.log("------Loading penguin commands.------")
  if (jsFiles.length <= 0) return console.log('Could not find any penguin commands!')
  jsFiles.forEach(file => {
    const cmd = require(`./commands/penguin/${file}`)
    console.log(`Loaded ${file}`)
    client.commands.set(cmd.name, cmd)
    if (cmd.aliases) cmd.aliases.forEach(alias => client.aliases.set(alias, cmd.name))
  })
